<template>
    <div>
        <table class="bannerTable" border="1" style="width: 100%">
            <thead>
                <tr>
                    <th>上传者</th>
                    <th>张数</th>
            ...
            <tbody v-for="mes in tableData" :key="mes">
                <tr v-for="(item,index) in mes" :key="item">
                    <td :rowspan="mes.length" v-if="index==0">{{item.from}}</td>
                    <td :rowspan="mes.length" v-if="index==0">{{mes.length}}张</td>
                    <td :rowspan="mes.length" v-if="index==0">{{item.time}}</td>
                    <td class="imbooder"><img :src="'http://127.0.0.1:3838/static/banner/'+item.name" width="50" height="50"/></td>
